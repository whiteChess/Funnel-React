...
import { getDefaultColors, createSVGElement, setAttrs } from "./graph";
...
class FunnelGraph {
  constructor(options) {
    ...
    this.gradientDirection =
      options.gradientDirection && options.gradientDirection === "vertical"
        ? "vertical"
        : "horizontal";
  ...
  validateData() {
    if (!this.values.length) {
      console.error(
        "Warning: The funnel component is waiting for a data property as an array."
      );
      console.log(
        "%c%s",
  ...
  static getValues(options) {
    ...
    if (typeof data === "object") return data.values;
  ...
  addLabels() {
    ...
    this.percentages.forEach((percentage, index) => {
      ...
      ReactDOM.render(
        this.renderLabel
          ? this.renderLabel(index, this.labels[index])
          : this.labels[index] || "",
        title
      );
      ...
      ReactDOM.render(
        this.renderValue
          ? this.renderValue(index, valueNumber)
      ...
      if (percentage) {
        const percentageText = this.customPercents
          ? `${this.customPercents[index].toString()}%`
          : `${percentage.toString()}%`;
        ReactDOM.render(
          this.renderPercentage
            ? this.renderPercentage(index, percentageText)
            : percentageText,
          percentageValue
        );
      }
